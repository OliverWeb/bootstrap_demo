					...
					data.message.sort(function (a, b) {
						...
						var server_li_html = ` <div class="ip-item">
				                    ...
				                    <a href="#" class="sk_item_pic_lk">
				                      ...
				                      <p>${value.hostname}</p>
				                      <p class="sk_item_name">${value.ip}</p>
				                    </a>
										  </div>
										  <div class="ip-right" style="display: inline-block;height: 90px;">
												  <div class="box" style=" display: -webkit-flex; ">
												  	<span><b>cpu核心数量：</b>${value.cpunumb}</span><span><b>硬盘大小：</b>${value.disktotal}</span><span><b>内存大小：</b>${value.ramtotal}</span><span><b>cpu型号（频率）：</b>${value.cputype}</span>
													</div>
												  <div class="row2 box" style="" display: -webkit-flex;">
												  	<span><b>物理cpu数量：</b>${value.totalofphysicalcpu}</span><span><b>硬盘当前使用量：</b>${value.diskused}</span><span><b>内存当前使用量：</b>${value.ramused}</span><span><b>网卡配置：</b>${value.ipconfig}</span>
													</div>
											</div>
