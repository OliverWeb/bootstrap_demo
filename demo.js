...
function PoolGeneralHtml() {
	$.ajax({
		...
		url: pageContext + "/mcrouter/getmcrouternormalconfig",               // 常规设置获取进行请求地址  变量：pageContext
...
function selectOption() {
	$.ajax({
		type: "get",
		dataType: "json", //服务端接收的数据类型
		url: pageContext + "/config/getMcrouterSharedPoolsList",               // 请求选择框中的所有选项option  变量：pageContext
		success: function (data) {
			if (data.status == "success") {
				if (data.message != "") {
					selectlist = JSON.parse(data.message);   //todo  展示所有的option 选择
				}
			} else {
				$('.tip-message').html(data.message);
				$('#messageModal').modal('show');
...
//遍历数组
	...
	if (router_selected_did.length == 0 || route_prefix_title == "") {
		...
		$.ajax({
			type: "post",
			dataType: "json", //服务端接收的数据类型
			url: pageContext + "/mcrouter/setmcrouternormal",               // 提交地址 变量：pageContext
			data: datas,
			success: function (data) {
				if (data.status == "success") {
