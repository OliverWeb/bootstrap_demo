$(function(){ 
    ...
    //跳到某页
    KingTable.prototype.gooto = function(toNum){
    ...
    KingTable.prototype.sortEvent = function(){
        ...
        var num=this.options.sortnum;
        sortContent.index=num;
        if( $.isArray(sortContent) && sortContent.length>0){
            $.each(sortContent,function(i,o){
                if(num< colNum){
                    tempThis.element.find("thead th").eq(num)
                        .off("click")
                        .on("click",function(){
    ...
    KingTable.prototype.searchEvent = function(){
        var _this = this;
        var  filter_data_by_search = function(Data,key_val){//通过搜索关键词筛选数据
