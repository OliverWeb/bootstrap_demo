var data=[];
$.get("../microserviceipcheck/list/",function(result){
	...
	if(result)
	{
		for(var i=0;i<result.length;i++)
		{
			...
			{
				hascheckbetaList.push(servicenameandversion);
			}
			...
			data[i]={checkbox:checkbox,servicenameandversion:servicenameandversion,ip:ip,port:port,displaystatus:displaystatus,displayweight:displayweight,displaybeta:displaybeta,displayconnections:displayconnections,displayfails:displayfails,createtime:createtime};
		}
		window.parent.hascheckbetaList=hascheckbetaList;
	}
	...
	$('#dg').datagrid({ loadFilter: pagerFilter }).datagrid({
		...
		columns:[[
		  		...
		  		{field:'createtime',title:'提交审核时间',width:'14%',sortable:true,sorter:function(a,b){
		  			a=a;
                	b=b;
                    if(a < b)
                        return -1;
			        ...
			        onOk: function(){
			        	$("input[name='checkbox']").each(function(){
							if($(this).prop("checked"))
							{
								...
								$.ajax({
							        type: "POST",
							        url: "../microserviceipchecked/insert?checkid="+value.id,
				        ...
				        onOk: function(){
				        	$("input[name='checkbox']").each(function(){
								if($(this).prop("checked"))
								{
									...
									$.ajax({
								        type: "GET",
								        url: "../microserviceipcheck/delete?id="+value.id,
								        contentType: "application/json",
								        dataType: "json",
								        async: false,
								        success: function (result) {     	
